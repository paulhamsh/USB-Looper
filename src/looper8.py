...
class Display:
    ...
    def draw_device(self, dev):
        self.font = ImageFont.truetype("/opt/looper/Ubuntu-Bold.ttf", 10, 0)
        self.draw.text((self.x[1], self.y[0]), dev,  font=self.font, fill = 255)

    def draw_numbers(self):
        self.font = ImageFont.truetype("/opt/looper/Ubuntu-Bold.ttf", 16, 0)
        for num in range(1, 5):
            self.draw.text((self.x[num], self.y[1]), str(num),  
                            font=self.font, fill = 255)

    def draw_bignum(self, num):
        self.font = ImageFont.truetype("/opt/looper/Ubuntu-Bold.ttf", 32, 0)
...
# Initialise buttons
button_stop = Button(23)
buttons = [Button(24), Button(25), Button(12), Button(13)]
...
high_frames = 0
frame = 0
is_playing = False  # either playing or recording 


def callback_pr(indata, outdata, frames, time, status):
    global recs, do_rec, frame, max_frame, is_playing
    if status:
        print(status)
...
with sd.Stream(channels=in_channels, samplerate = samplerate, blocksize = blocksize, callback=callback_pr):
    while True:
        ...
        button_stop.update()
        b_status = button_stop.status
        if b_status != Button.NO_PRESS:
            if b_status == Button.SHORT_PRESS:
                if is_playing:
                    is_playing = False
                    frame = 0
                    screen.draw_bar(0)
                    screen.draw_box(True)
                else:
                    if high_frames > 0:
                        is_playing = True 
                        screen.draw_box(False)
            elif b_status == Button.LONG_PRESS:
                is_playing = False
                screen.draw_box(True)
                screen.draw_bar(0)
                high_frames = 0
